...
from Libs_Functions_Alpiq import *


# ------------- Settings --------------

page_title = 'Alpiq App'
page_description = 'This App automates predictive data analysis for the PJM Hourly Energy Consumption Dataset with the [scikit-learn](https://scikit-learn.org/stable/index.html) API. \
It is a functional demo for Alpiq and automates the process steps for a small data science project or a first shot at model selection. \
...
image = Image.open('images/header3.png')
...
st.write("This is a publicly available dataset for machine learning. The hourly power consumption data comes from PJM's website and are in megawatts (MW).")

with st.expander("See Data description"):
    st.write("""
        PJM Interconnection LLC (PJM) is a regional transmission organization (RTO) in the United States. It is part of the Eastern Interconnection grid operating an electric transmission system serving all or parts of Delaware, Illinois, Indiana, Kentucky, Maryland, Michigan, New Jersey, North Carolina, Ohio, Pennsylvania, Tennessee, Virginia, West Virginia, and the District of Columbia.
        
        PJM East Region: 2014-2018 (PJME): estimated energy consumption in Megawatts (MW)
    """)
...
csv_options = {
    ...
    'Energy Consumption Hourly': ['local_data/PJME_hourly.csv' , ','],
...
use_csv_name = 'Energy Consumption Hourly'
...
fig = px.line(df, x = 'Datetime', y = 'Energy_Consumption_MW',
                    title= 'Hourly Energy Consumption')
fig.update_layout(xaxis_title= 'Date and Hour', yaxis_title= 'Energy Consumption in Mega Watts').update_xaxes(
...
# with col1_plot:
#     us_x_axis = st.selectbox('select x-axis', axis_options, index = 0)


# if us_plot_type != 'Histogramm':
#     with col2_plot:
#         us_y_axis = st.selectbox('select y-axis', axis_options, index = 1)
...
st.write("Here you can choose what date range you wnat to predict. The data is then split into a training and a test set.\
          All results will be reported on the test set (prediction period) with data the algorithm hasn't seen in training. To avoid data leakage by train-test contamination all further \
         preprocessing steps will be executed separately for the training and test set.")

st.write("")

st.subheader('Select Date Range for Prediction')
...
with st.expander("See Preprocessing Options"):

    st.write('')
    st.subheader('Data Preprocessing')
    st.write("In a first step the information from the date is extracted (month, year, day of week etc.). This data set is actually allready quite clean. Some common preprocessing steps are still shown. It is assumed, that feature engineering steps like binning and ordinal encoding are allready applied to the data. \
...
st.write('Here we tell the models what we want to predict (Energy Consumption) and what information they should use to do so. \
         You can just leave it in the default configuration and use all available information or play around with different inputs.')
st.write('')
...
with st.expander("See Selected Variables"):
...
descr_model_launch = 'Here is a selection of algorithms that can be applied to predict the hourly energy consumption and compared on their performance metrics. \
    You can just start them with the default configuration and all models selected. It needs a bit of time to calculate.\
...
with st.expander("See Scaling Options"):

    st.subheader('Scaling')
    st.write('Here you could choose another scaler if you would want to.')
...
# only if y is a number type
if us_y_var in reg_cols and len(cat_cols_x) == 0 and len(us_x_var) > 0 and 1 == 2:
